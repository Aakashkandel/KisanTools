...
<div class="flex flex-col w-full max-w-3xl mx-auto mt-10">
    ...
    <div class="bg-gray-100 p-6 rounded-lg shadow-lg">
        <h2 class="text-2xl font-semibold text-green-800 mb-4">Create Category</h2>
        <form action="{{route('admin.category.store')}}" method="POST">
            @csrf
        
            <div class="mb-4">
                <label for="name" class="block text-sm font-medium text-gray-600">Category Name</label>
                <input type="text" name="name" value="{{old('name')}}" id="category_name" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" >
                @error('name')
            ...
            <div class="mb-4">
                ...
                <input type="number" name="priority" value="{{old('priority')}}" id="priority" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" >
                @error('priority')
                <p class="text-red-500 mt-2">{{$message}}</p>
            @enderror
            </div>
            ...
            <div class="flex justify-end">
                ...
                <button type="submit" class="px-4 py-2 bg-blue-600 mx-2 text-white rounded-lg shadow-md">Create Category</button>
            </div>
        </form>
    </div>
</div>
